...
void buyTimeInCoordinates(Player *customer, Field ***polygon, unsigned int x, unsigned int y) {
    ...
    unsigned int val = getStartValueOfType(type);
    ...
    if (type == '1') {
       install(customer, polygon, x, y, type, new crusher(customer->getSide()[0], x, y));
     
    }
    else if (type == '2') {
        install(customer, polygon, x, y, type, new medic(customer->getSide()[0], x, y));
    } 
    else if (type == '3') {
        install(customer, polygon, x, y, type, new armory(customer->getSide()[0], x, y));
    }
    else if (type == '0'){
        return;
    } else {
...
void buyTime(Player *customer, Field ***polygon) {
    ...
    std::cout << customer->getName() << "\nEnter coordinates(from 0 to " << polygonSize - 1
              << ") where you want to install(-1 if you want to go back): ";
    std::cin >> x;
    if (x == -1)
        return;
    std::cin >> y;
    if (x < 0 || y < 0 || x >= polygonSize || y >= polygonSize) {
        std::cout << "Coordinates must be from 0 to " << polygonSize << std::endl;
        buyTime(customer, polygon);
        return;
    }
    if (!polygon[x][y]->isEmpty()) {
